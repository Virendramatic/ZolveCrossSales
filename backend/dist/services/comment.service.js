...
class CommentService {
     ...
     */
    static async create(leadId, data, authorId, authorName, authorRole) {
        try {
            // Verify lead exists
     ...
     */
    static async listByLead(leadId, options = {}) {
        try {
            ...
            const comments = await index_1.prisma.comment.findMany(query);
     ...
     */
    static async update(commentId, content, userId, userName, userRole) {
        try {
     ...
     * @throws 404 if comment not found
     */
    static async delete(commentId, userId, userName, userRole) {
        try {
            // Get comment to check permissions
            const comment = await index_1.prisma.comment.findUnique({
                where: { id: commentId },
            ...
            // Soft delete comment
            await index_1.prisma.comment.update({
                where: { id: commentId },
                data: {
                    deletedAt: new Date(),
                },
