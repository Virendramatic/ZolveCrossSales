...
class ProductService {
     ...
     */
    static async create(leadId, productType, ownerUserId) {
        try {
            // Verify lead exists
            ...
            const existingCount = await index_1.prisma.productInstance.count({
                where: {
                    leadId,
                    productType,
                    archivedAt: null,
            ...
            if (existingCount >= maxInstances[productType]) {
                const error = new Error(`Lead already has maximum number of ${productType} instances`);
                error.statusCode = 400;
                error.code = 'PRODUCT_LIMIT_EXCEEDED';
                throw error;
            }
            // Create product instance
            const product = await index_1.prisma.productInstance.create({
                data: {
                    leadId,
                    productType,
     ...
     */
    static async getById(productId, userId, userRole) {
        try {
            const product = await index_1.prisma.productInstance.findUnique({
                where: { id: productId },
                select: {
                    id: true,
                    productCode: true,
            ...
            if (userRole === 'COUNSELOR' && product.ownerUserId !== userId) {
                const error = new Error('You do not have permission to access this product');
     ...
     */
    static async listByLead(leadId, userId, userRole) {
        try {
            // Verify lead exists
            const lead = await index_1.prisma.lead.findUnique({
                where: { id: leadId },
            ...
            const products = await index_1.prisma.productInstance.findMany({
                where: {
                    leadId,
                    archivedAt: null,
                },
     ...
     */
    static async updateStatus(productId, status, userId, userRole) {
        try {
            ...
            // Update status
            const updatedProduct = await index_1.prisma.productInstance.update({
                where: { id: productId },
                data: {
                    status,
                    updatedAt: new Date(),
    ...
    static async delete(productId, userId, userRole) {
        try {
            ...
            // Soft delete
            await index_1.prisma.productInstance.update({
                where: { id: productId },
                data: {
                    archivedAt: new Date(),
                },
     ...
     */
    static async getLeadsByProductType(productType, userId, userRole, options = {}) {
        try {
