...
export interface MatchRecommendation {
  ...
  factors: {
    loanAmountFit: number; // 0-30
    courseFit: number; // 0-20
    countryFit: number; // 0-20
    roiFit: number; // 0-15
    eligibilityScore: number; // 0-15
  };
...
const LENDER_POOL: LenderProfile[] = [
  {
    id: 'lender-1',
...
export class MatcherService {
  ...
  private static calculateCourseFit(
    universityRanking: number | undefined,
    lender: LenderProfile
  ): number {
    ...
    if (ranking <= 100) return 20;
    if (ranking <= 300) return 16;
    if (ranking <= 500) return 12;
    if (ranking <= 1000) return 8;
    return 4;
  }
  ...
  private static calculateCountryFit(
    ...
    if (!country || !lender.acceptedCountries) return 10; // Neutral score
   ...
   */
  private static calculateROIFit(lender: LenderProfile): number {
   ...
   */
  private static calculateEligibilityScore(lender: LenderProfile): number {
    let score = 15;

    // Penalty for high processing fees
    const fee = lender.processingFeePercentage || 0;
    if (fee > 2.5) score -= 4;
    else if (fee > 2) score -= 2;
  ...
  static async generateRecommendations(
    ...
    const alreadyMatched = await prisma.lenderApplication.findMany({
    ...
    const recommendations: MatchRecommendation[] = LENDER_POOL.filter((l) => !matchedNames.has(l.name))
      .map((lender) => {
        ...
        const countryFit = this.calculateCountryFit(
          loan.targetCountry || undefined,
          lender
        );
        ...
        const matchScore = Math.min(
          100,
          loanAmountFit + courseFit + countryFit + roiFit + eligibilityScore
        );
        ...
        if (loanAmountFit >= 25) {
          ...
          reasoning.push(
            `Loan amount outside lender range (₹${(lender.minLoanAmount || 0) / 100000}L - ₹${(lender.maxLoanAmount || 0) / 100000}L)`
          );
        }

        if (countryFit === 20) {
          reasoning.push(
            `${loan.targetCountry || 'Target country'} is in lender's accepted countries`
          );
        } else if (countryFit === 0 && loan.targetCountry) {
          reasoning.push(
            `${loan.targetCountry} is not in lender's accepted countries`
          );
        }

        if (eligibilityScore < 10) {
          reasoning.push(
            `High processing fee (${lender.processingFeePercentage}%)`
          );
        }

        reasoning.push(
          `ROI range: ${lender.minROI}% - ${lender.maxROI}%`
        );

        let recommendation = 'CONSIDER';
        if (matchScore >= 80) recommendation = 'HIGHLY_RECOMMENDED';
        else if (matchScore >= 60) recommendation = 'RECOMMENDED';
        else if (matchScore < 30) recommendation = 'NOT_RECOMMENDED';
   ...
   */
  static async getLenderPool(): Promise<LenderProfile[]> {
