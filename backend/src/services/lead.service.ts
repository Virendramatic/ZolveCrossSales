...
export class LeadService {
  ...
  static async update(
    ...
    try {
      ...
      if (userRole === 'COUNSELOR' && lead.currentOwnerId !== userId) {
        const error = new Error('You do not have permission to update this lead') as AppError;
        error.statusCode = 403;
        error.code = 'FORBIDDEN';
        throw error;
      }
      ...
      Object.keys(updateData).forEach(key => {
        if (lead[key as keyof typeof lead] !== updateData[key]) {
          changes.push({
            fieldName: key,
            oldValue: lead[key as keyof typeof lead],
  ...
  static async delete(
    ...
    try {
      ...
      if (userRole === 'COUNSELOR' && lead.currentOwnerId !== userId) {
        const error = new Error('You do not have permission to delete this lead') as AppError;
        error.statusCode = 403;
        error.code = 'FORBIDDEN';
        throw error;
      }
  ...
  static async updateCallStatus(
    ...
    try {
      ...
      if (!validStatuses.includes(globalCallStatus)) {
        const error = new Error(`Invalid call status: ${globalCallStatus}`) as AppError;
      ...
      const updatedLead = await prisma.lead.update({
        where: { id: leadId },
        data: {
          globalCallStatus: globalCallStatus as any,
        },
        select: {
          id: true,
          leadCode: true,
   ...
   * @throws 404 if lead not found
   */
  static async setRescheduleDate(
    leadId: string,
    rescheduleDate: Date,
    userId: string,
    userRole: string
    ...
    try {
      ...
      if (rescheduleDate <= now) {
        const error = new Error('Reschedule date must be in the future') as AppError;
        error.statusCode = 400;
