...
export class DocumentService {
  ...
  static async uploadDocument(
    ...
    if (input.fileSize > 10 * 1024 * 1024) {
      const error = new Error('File size exceeds 10MB limit') as AppError;
    ...
    const allDocuments = await prisma.documentSubmission.findMany({
      where: { documentRequestId: input.documentRequestId },
    });

    const allSubmitted = allDocuments.every(
  ...
  static async approveDocument(
    ...
    if (userRole !== 'ADMIN') {
      const error = new Error('Only admins can approve documents') as AppError;
      error.statusCode = 403;
      error.code = 'FORBIDDEN';
      throw error;
    }
    ...
    if (document.documentRequest.educationLoanId !== loanId) {
      const error = new Error('Document does not belong to this loan') as AppError;
      error.statusCode = 400;
    ...
    if (allApproved) {
      ...
      if (loan.loanStage === 'DOCS_PENDING') {
        await prisma.educationLoanApplication.update({
          where: { id: loanId },
  ...
  static async rejectDocument(
    ...
    if (userRole !== 'ADMIN') {
      const error = new Error('Only admins can reject documents') as AppError;
      error.statusCode = 403;
      error.code = 'FORBIDDEN';
      throw error;
    }
  ...
  static async getDocumentVersions(
    ...
    const versions = await prisma.documentSubmission.findMany({
      where: {
